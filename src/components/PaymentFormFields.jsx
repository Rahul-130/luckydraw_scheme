...
import { CurrencyRupee, ReceiptLong, CalendarMonth, Add, Delete, Person } from '@mui/icons-material';
import PreviewCopy from './PreviewCopyComponent';


const PaymentFormFields = ({ formState, onFormChange, isMonthDisabled = false, user, bookTotalAmount }) => {
  ...
  return (
    ...
    >
      ...
      <Stack spacing={3}>
        ...
        {/* Agent Name - Now always shows the logged-in user's name */}
        <Alert severity="info" icon={<Person />}>
           Recording payment as: <strong>{user?.name}</strong>
        </Alert>
        ...
        {/* Preview + copy */}
        <PreviewCopy
          formState={{ ...formState, amount: totalAmount, paymentType: displayPaymentType, agentName: user?.name }}
          fields={[
